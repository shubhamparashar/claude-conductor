...
- `model-routing-context` (SessionStart) — injects a delegation ladder (collapsed to a one-line pointer when a `model-routing*.md` rule file already exists under `~/.claude/rules`, `~/.claude/rules-detail`, or `$CONDUCTOR_RULES_DIR`): spawned agents run at the lowest capable tier (haiku → sonnet → opus-high), the main loop keeps orchestration, final synthesis, and strategy, and subagent prompts stay narrow and self-contained so your strategic context never leaks into worker prompts.
- `memory-nudge` (UserPromptSubmit) — pure relay for the one-shot flags dropped by the Stop hooks (post-task reflection, unchecked goal contract). No counters, no per-prompt writes, silent unless a flag exists.
...
- `conductor-doctor` (SessionStart) — self-watcher that only speaks up on misbehavior: fast checks over the plugin's own moving parts and the surrounding setup (hook references, skill frontmatters, session-index integrity, double registration, skill shadowing, node runtime, automation-job logs, settings hook paths that are relative or missing, hooks that invoke a skill turned off in `skillOverrides`, boundary actions pre-approved in `permissions.allow`, KeepAlive launchd jobs exiting non-zero plus runaway job logs, a SKILL.md pasted into an always-loaded CLAUDE.md, and a plugin cache behind the local clone). Silent while healthy; on failure it upserts an OPEN entry in `~/.claude/conductor-report.md` and writes the SAME entry back to RESOLVED once the problem clears — a self-maintaining bug report, no duplicates. **GitHub mirroring (optional):** set `CONDUCTOR_ISSUE_REPO=owner/repo` (or pass as the hook's second arg) and each new failure opens one GitHub issue, linked in the report entry; recovery closes that same issue with a comment. Requires `gh` authenticated; all GitHub calls are best-effort and never break the health check.
